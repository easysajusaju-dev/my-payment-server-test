export async function POST(req) {
  ...
  // ✅ [1] 결제 취소 또는 실패 처리
  // authResultCode가 "0000"이 아닌 경우 → 결제취소/실패로 간주
  ...
  if (result.resultCode === "0000") {
    const redirectUrl = `https://easysajusaju-dev.github.io/thankyou.html?oid=${orderId}&product=${encodeURIComponent(
      goodsName
    )}&price=${amount}`;

    // ✅ Logger.gs로 결제완료 상태 전송
    try {
      await fetch(
        "https://script.google.com/macros/s/AKfycbz_SRAMhhOT396196sgEzHeDMNk_oF7IL-M5BpAReKum04hVtkVYw0AwY71P4SyEdm-/exec",
        {
